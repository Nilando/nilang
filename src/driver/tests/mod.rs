...
use super::{run_script, Config};

fn fixture_test(test_name: &str) {
    let full_program_path = format!("./src/driver/tests/test_programs/{}.nl", test_name);
    let fixture_folder = format!("./src/driver/tests/fixtures/{}", test_name);
    ...
    if let Err(err) = run_script(config) {
