...
impl ParseError {
    ...
    pub fn render(&self) -> String {
        ...
        for spanned_err in self.items.iter() {
            ...
            let location_line = format!("  File \"{path}\", line: {line}:{end}\n");
        ...
        result.push_str(&format!("Failed to parse due to the previous {} errors", self.items.len()));
        result
    }
