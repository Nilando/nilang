...
fn display_parse_errors(stdout: &mut std::io::Stdout, input: &str, errors: &[Spanned<ParseError>], file_name: Option<String>) {
        ...
        while error_idx < errors.len() && errors[error_idx].span.0 < line_end {
            ...
            if error.span.0 >= line_start && error.span.0 < line_end {
                ...
                if let Some(ref file_name) = file_name {
                    write!(stdout, "{}-->{} {}:{}:{}\r\n", blue, reset, file_name, line_num, error.span.0 - line_start).unwrap();
                    write!(stdout, "   {}|{}\r\n", blue, reset).unwrap();
